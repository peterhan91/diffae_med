...
def prepare_data(data_type, path, out, n_worker, sizes, quality, chunksize):
    ...
    df = pd.read_csv(os.path.join(path, 'UKA_dataset_all_splits.csv'))
    df = df[df['split'] == 'train']
    files = df['Anforderungsnummer'].tolist()
    filenames = [os.path.join(path, str(f)+'.jpg') for f in files]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quality', type=int, help='output jpeg quality', default=100)

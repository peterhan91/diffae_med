...
class PatchDataset(Dataset):
    def __init__(self, path_to_images, fold='test', sample=0, transform=None):
        ...
        self.df = pd.read_csv('../Stanford_MIT_CHEST/MIMIC-CXR-v2.0/csvs/mimicfrontal_label.csv')
        # self.df = pd.read_csv('./datasets/chexpert/label.csv')
        self.fold = fold
        ...
        self.df = self.df.set_index('dicom_id')
    ...
    def __getitem__(self, idx):
        image = Image.open(
            os.path.join(self.path_to_images, self.df.index[idx]+'.jpg')
...
class Tester():
    def __init__(self, log_folder, conf, cls_conf, device='cuda:0'):
        ...
        self.total_model = ClsModel(cls_conf)
        state = torch.load(f'checkpoints/{cls_conf.name}/last.ckpt',
                            map_location='cpu')
        print('latent step:', state['global_step'])
        self.total_model.load_state_dict(state['state_dict'], strict=False)
        self.total_model.to(device)

    def test(self, loader):
        label_list, pred_list = [], []
        with torch.no_grad():
            self.total_model.ema_model.eval()
            self.total_model.ema_classifier.eval()
            for data, label in loader:
                data, label = tensor2cuda(data), tensor2cuda(label)
                latent = self.total_model.ema_model.encoder(data)
                latent = self.total_model.normalize(latent)
                output = self.total_model.ema_classifier.forward(latent)                
...
if __name__ == '__main__':
    ...
    cls_conf = mimic256_autoenc_cls_finetune()
    tester = Tester(conf=conf, cls_conf=cls_conf, log_folder=f'checkpoints/{cls_conf.name}/')
    te_dataset = PatchDataset(path_to_images='../Stanford_MIT_CHEST/MIMIC-CXR-v2.0/mimic-cxr',
                            # path_to_images='../CheXpert_Dataset/images_256/images',
                            fold='test',
